...
function DashboardSideBar() {
    ...
    const product = [
        "products-dashboard" , "products-drafts" , "products-released" , "products-scheduled"
    ]
    return (
        <div className='sm:flex h-full bg-white dark:bg-app-neutral-700 xl:min-w-[340px] hidden xl:max-w-[340px] p-6 gap-1 flex-col'>
            ...
            {product.includes(selectedItem)  &&
                <div className='w-[255px] flex flex-col gap-2 ml-auto'>
                    <ListItem onClick={() => {
                        setSelected("products-dashboard")
                    }} selectedItem={selectedItem} item='products-dashboard' text='Dashboard' iconRight={<ChevronDownIcon className='' />} className='font-[600] text-gray-600' />
                    <ListItem onClick={() => {
                        setSelected("products-drafts")
                    }} selectedItem={selectedItem} item='products-drafts' text='Drafts' iconRight={<div className='w-6 h-6 font-bold flex items-center justify-center rounded-lg bg-secondary-1'>2</div>} className='font-[600] text-gray-600' />
                    <ListItem onClick={() => {
                        setSelected("products-released")
                    }} selectedItem={selectedItem} item='products-released' text='Released' className='font-[600] text-gray-600' />
                    <ListItem onClick={() => {
                        setSelected("products-scheduled")
                    }} selectedItem={selectedItem} item='products-scheduled' text='Scheduled' iconRight={<div className='w-6 h-6 font-bold flex items-center justify-center rounded-lg bg-purple-300'>8</div>} className='font-[600] text-gray-600' />
                </div>
            }
            <ListItem onClick={() => {
