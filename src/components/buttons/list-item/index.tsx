  ...
  return (
    ...
    >
      <div className="flex gap-3">
        {iconLeft && iconLeft}
        {text && !isMobile ? (
          <div className={`text-${textColor} hidden xl:flex`}>{text}</div>
        ) : (
          <div className={`text-${textColor} flex`}>{text}</div>
        )}
      </div>
      {iconRight && !isMobile ? (
        <div
          className={`${
            selectedItem === item && isDropdown
              ? `${isRotatable ? "-rotate-90" : ""} hidden xl:flex`
              : "hidden xl:flex"
          }`}
        >
          {iconRight}
